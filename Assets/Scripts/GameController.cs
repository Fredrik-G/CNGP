...
public class GameController : MonoBehaviour
{
	...
	public void DisableScripts()
	{
		...
		this.GetComponent<TopDownController>().Disable = true;
		this.GetComponent<ActionBarController>().enabled = false;
		Debug.Log (this.GetComponent<ActionBarController>().enabled + " DisableScripts");
			...
			if (gameOver) {
				if (GetComponent<PhotonView> ().isMine) {
					...
					if (listOfPlayersLength >= 1) {
						textPlayerOne.GetComponent<Text> ().text = "1. " + listOfPlayers [0].name;
					...
					if (listOfPlayersLength >= 2) {
						textPlayerTwo.GetComponent<Text> ().text = "2. " + listOfPlayers [1].name + " ";
					...
					if (listOfPlayersLength >= 3) { 
						textPlayerThree.GetComponent<Text> ().text = "3." + listOfPlayers [2].name;
					...
					if (listOfPlayersLength >= 4) {
						textPlayerFour.GetComponent<Text> ().text = "4." + listOfPlayers [3].name;
